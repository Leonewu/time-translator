...
VIP 用户输入 `emma` 后，Popup Logo 和浏览器工具栏图标会实时切换为 VIP 版本。Tooltip 中的“接案順心!”按钮还藏有一个彩蛋：连续点击时 Tooltip 会进入半透明状态，并在停止点击约 3 秒后缓慢恢复；每次完成 10 秒内点击 10 次都会喷出爱心，文案按 `🧩 emma~`、`对不起！😣` 循环。长按超过 1 秒会让按钮倾斜颤抖，松手后使用普通蓄力彩带并显示紫色 `🧩 emma~`。
